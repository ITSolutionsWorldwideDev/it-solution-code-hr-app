    ...
    <div className="rounded-[30px] border border-[#dfe9f3] bg-white p-6 shadow-sm">
      ...
      <form className="mt-6 space-y-5" onSubmit={handleSubmit}>
        ...
        {generatedSkills.length > 0 ? (
          <div className="rounded-[22px] bg-[#f4f8fd] px-4 py-4">
            <p className="text-sm font-semibold text-[#27405f]">AI-extracted required skills</p>
            <div className="mt-3 flex flex-wrap gap-2">
              {generatedSkills.map((skill) => (
                <span
                  key={skill}
                  className="rounded-full bg-white px-3 py-1.5 text-sm font-medium text-[#2b76ab]"
                >
                  {skill}
                </span>
              ))}
        ...
        {errorMessage ? (
          <div className="rounded-[18px] border border-[#f0d5d7] bg-[#fff7f8] px-4 py-3 text-sm text-[#a65765]">
            {errorMessage}
          </div>
        ) : null}
