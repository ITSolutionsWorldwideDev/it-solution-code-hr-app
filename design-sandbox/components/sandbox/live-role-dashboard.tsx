    ...
    const loadHrDashboard = async () => {
      try {
        const [summaryResponse, activityResponse] = await Promise.all([
          apiRequest<HRDashboardSummaryApiRecord>({ path: "/dashboard/hr-summary" }),
          apiRequest<HRDashboardActivityResponseApiRecord>({ path: "/dashboard/hr-activity" }),
        ]);
        if (!cancelled) {
      ...
      <section className="grid gap-4 xl:grid-cols-[1.78fr_1fr] xl:grid-rows-[auto_auto]">
        ...
        <div className="space-y-4">
          <Panel className="space-y-4 rounded-[28px] p-4">
            <h3 className="text-[1.2rem] font-semibold text-white">Quick Actions</h3>
